...
export default function Analytics() {
  ...
  return (
    <div className="an-root" style={{ width: '100%', padding: '36px', backgroundColor: '#F8F5F0', minHeight: '100vh', boxSizing: 'border-box' }}>
      <style>{`
        ...
        .an-insights-row { display: grid; grid-template-columns: 320px 1fr 320px; gap: 24px; }
        @media (max-width: 1360px) {
          .an-insights-row { grid-template-columns: 1fr; }
        }
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 32 }}>
        {PLAT_TABS.map(p => <button key={p} style={pill(platTab === p)} onClick={() => setPlatTab(p)}>{p === 'All Platforms' ? 'Global' : p}</button>)}
      </div>
          ...
          <div style={{ height: 320, width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="tiktok"    stroke="#7A9A6E" strokeWidth={5} dot={false} activeDot={{ r: 8, stroke: '#fff', strokeWidth: 4 }} />
      ...
      {/* Insights Row */}
      <div className="an-insights-row">
        {/* Distribution */}
        <Card>
          <h4 style={{ fontSize: 16, fontWeight: 800, color: '#2B2218', margin: '0 0 6px' }}>Platform Distribution</h4>
        ...
        <Card>
          <h4 style={{ fontSize: 16, fontWeight: 800, color: '#2B2218', margin: '0 0 6px' }}>Content Hall of Fame</h4>
          <p style={{ fontSize: 13, color: '#7A7068', margin: '0 0 24px' }}>Top posts by algorithmic resonance</p>
          <div style={{ display: 'flex', borderBottom: '1px solid #ECE6DE', paddingBottom: 12, marginBottom: 12 }}>
            <span style={{ flex: 3.5, fontSize: 12, fontWeight: 800, color: '#B0A89C' }}>CONTENT</span>
              ...
              <div key={idx} style={{ display: 'flex', padding: '14px 0', borderBottom: idx === contentTable.length - 1 ? 'none' : '1px solid #F4F0EB', alignItems: 'center' }}>
                <div style={{ flex: 3.5 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span style={{ fontSize: 10, fontWeight: 900, padding: '3px 8px', borderRadius: 6, background: platColor[item.platform]?.bg || '#EEE', color: platColor[item.platform]?.text || '#666' }}>{item.platform.toUpperCase()}</span>
                ...
                <div style={{ flex: 1, textAlign: 'right' }}>
                  <span style={{ fontSize: 16, fontWeight: 900, color: vColor(item.v) }}>{item.v}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Health Scores */}
        <Card>
          <h4 style={{ fontSize: 16, fontWeight: 800, color: '#2B2218', margin: '0 0 6px' }}>Algorithmic Signals</h4>
          ...
          <div style={{ marginTop: 28, padding: '16px', background: 'rgba(192,90,56,0.05)', borderRadius: 16, border: '1px solid #FEE8DC' }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: '#7A7068', lineHeight: 1.6 }}>Hook strength is trending up. Post a TikTok Story highlighting these metrics to boost secondary reach loops.</p>
          </div>
        </Card>
      </div>
    </div>
